...
export const fontDmSans = createFont({
  family: 'DM Sans',
  weight: {
    regular: '400',
    1: '400',
  ...
  face: {
    // Android uses filename, iOS uses PostScript name (configured in app.config.js with expo-font plugin)
    '400': { normal: Platform.select({ ios: 'DMSans-Regular', android: 'DMSans_400Regular' }) },
    '500': { normal: Platform.select({ ios: 'DMSans-Medium', android: 'DMSans_500Medium' }) },
    '600': { normal: Platform.select({ ios: 'DMSans-SemiBold', android: 'DMSans_600SemiBold' }) },
    '700': { normal: Platform.select({ ios: 'DMSans-Bold', android: 'DMSans_700Bold' }) },
  },
})
