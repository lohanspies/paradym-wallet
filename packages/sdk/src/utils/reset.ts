...
export const reset = async (paradym?: ParadymWalletSdk, walletId?: string) => {
  ...
  const effectiveWalletId = paradym?.walletId ?? walletId
  if (effectiveWalletId) {
    const walletDirectory = `${fs.dataPath}/wallet/${effectiveWalletId}`
